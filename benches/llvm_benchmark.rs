...
fn bench_mg_with_llvm(path: &Path, compile: bool, run: bool) -> Result<()> {
    if compile {
        // 1) Read .mg source
...
fn bench_compilation(c: &mut Criterion) {
    ...
    for mg_file in mg_files {
        ...
        let test_name = format!("{} (LLVM compilation)", mg_file.file_stem().unwrap().to_string_lossy());
        ...
        c.bench_function(&test_name, |b| {
            b.iter(|| {
                ...
                let _ = bench_mg_with_llvm(black_box(mg_file.as_path()), true, false);
            });
        });
    }
}
...
fn bench_runtime(c: &mut Criterion) {
    // Configure Criterion to sample only 10 executions
    let mut group = c.benchmark_group("LLVM Execution");
    ...
    for mg_file in mg_files {
        // Create a custom benchmark ID based on the filename
        let test_name = format!("{} (LLVM execution)", mg_file.file_stem().unwrap().to_string_lossy());

        group.bench_function(&test_name, |b| {
            b.iter(|| {
                // We call `compile_mg_llvm` once per iteration
                // In a real scenario, you might do more steps (link, run, etc.)
                let _ = bench_mg_with_llvm(black_box(mg_file.as_path()), false, true);
            });
        });
    }

    // Drop the group to ensure all benchmarks are run
    group.finish();
}
